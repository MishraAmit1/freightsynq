...
const AutoFitBounds: React.FC<{ 
    ...
    useEffect(() => {
        ...
        } else if (mode === 'random' && randomSearch && randomSearch.toll_crossings.length > 0) {
            const bounds = randomSearch.toll_crossings.map(c => [c.latitude, c.longitude] as [number, number]);
    ...
    return (
        <div className="relative h-[600px] w-full rounded-lg overflow-hidden border border-border z-0">
            ...
            >
                ...
                {mode === 'fleet' && groupedVehicles && groupedVehicles.map((group, index) => (
                    <Marker
                        key={index}
                        position={[group.lat, group.lng]}
                        icon={group.count > 1 ? createFleetClusteredIcon(group.count) : createFleetSingleIcon()}
                    >
                        <Popup>
                ...
                {mode === 'random' && selectedRandomSearch && (
                    <>
                        {selectedRandomSearch.search_type === 'live' ? (
                            selectedRandomSearch.last_latitude && selectedRandomSearch.last_longitude && (
                                <Marker
                                    position={[selectedRandomSearch.last_latitude, selectedRandomSearch.last_longitude]}
                                ...
                                >
                                    <Popup>
                                        <div className="p-3 min-w-[200px]">
                                            ...
                                            <p className="font-mono font-semibold mb-2">{selectedRandomSearch.vehicle_number}</p>
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                ...
                                                {selectedRandomSearch.last_crossing_time 
                                                    ? getTimeAgo(selectedRandomSearch.last_crossing_time)
                                                    : 'Unknown time'
                                                }
                                            </p>
                            ...
                            <>
                                ...
                                {selectedRandomSearch.toll_crossings.length > 1 && (
                                    <>
                                        <Polyline
                                            positions={selectedRandomSearch.toll_crossings.map(c => [c.latitude, c.longitude])}
                                            color="#f97316"
                                            weight={4}
                                            opacity={0.7}
                                        />
