...
const VerifiedBadges: React.FC<{ party: Party }> = ({ party }) => {
  return (
    <div className="flex items-center gap-1 min-[2000px]:gap-1.5">
      {party.has_gst_verified && (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
              <div className="w-5 h-5 min-[2000px]:w-6 min-[2000px]:h-6 rounded-full bg-green-100 flex items-center justify-center">
                <Check className="w-3 h-3 min-[2000px]:w-4 min-[2000px]:h-4 text-green-600" />
              </div>
            </TooltipTrigger>
            <TooltipContent>
              <p className="text-xs min-[2000px]:text-sm">
      ...
      {party.has_pan_verified && (
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger>
              <div className="w-5 h-5 min-[2000px]:w-6 min-[2000px]:h-6 rounded-full bg-blue-100 flex items-center justify-center">
                <FileText className="w-3 h-3 min-[2000px]:w-4 min-[2000px]:h-4 text-blue-600" />
              </div>
            </TooltipTrigger>
            <TooltipContent>
              <p className="text-xs min-[2000px]:text-sm">
      ...
      {!party.has_gst_verified &&
        !party.has_pan_verified &&
        !party.has_documents && (
          <span className="text-xs min-[2000px]:text-sm text-muted-foreground">
            —
          </span>
        )}
    </div>
...
const ImportModal: React.FC<{
  ...
  const validateData = async (data: any[]) => {
    ...
    data.forEach((row) => {
      ...
      if (row.gst_number)
        gstNumbers.push(row.gst_number.toString().trim().toUpperCase());
    ...
    if (phoneNumbers.length > 0) {
      const { data: existingData } = await supabase
        ...
        .or(
          `phone.in.(${phoneNumbers.join(",")}),gst_number.in.(${gstNumbers
            .filter((g) => g)
            .join(",")})`,
        );
    ...
    data.forEach((row, index) => {
      ...
      const processedRow: ImportRow = {
        ...
        party_type:
          (row.party_type?.toString().trim().toUpperCase() as any) || "BOTH",
        status:
          (row.status?.toString().trim().toUpperCase() as any) || "ACTIVE",
        is_billing_party:
          row.is_billing_party?.toString().trim().toUpperCase() === "TRUE",
      };
      ...
      if (
        processedRow.gst_number &&
        processedRow.gst_number.length > 0 &&
      ...
      if (
        processedRow.pan_number &&
        processedRow.pan_number.length > 0 &&
      ...
      if (
        !["CONSIGNOR", "CONSIGNEE", "BOTH"].includes(processedRow.party_type!)
      ) {
      ...
      if (existingNames.includes(processedRow.name.toLowerCase())) {
        ...
        processedRow.gst_number &&
        existingGSTs.includes(processedRow.gst_number)
      ) {
        duplicates.push({
          row: processedRow,
          field: "gst_number",
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        ...
        const { error } = await supabase.from("parties").insert(dataToInsert);
    ...
    toast({
      ...
      description: `Successfully imported ${successCount} parties${
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl min-[2000px]:max-w-6xl max-h-[90vh] bg-gradient-to-br from-background via-background to-muted/5">
        <DialogHeader className="border-b pb-4 min-[2000px]:pb-6">
          <DialogTitle className="text-xl min-[2000px]:text-2xl flex items-center gap-2 min-[2000px]:gap-3">
            <Upload className="w-5 h-5 min-[2000px]:w-6 min-[2000px]:h-6 text-primary" />
            Import Parties
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4 min-[2000px]:space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                className="h-9 min-[2000px]:h-11 min-[2000px]:text-base hover:bg-primary/10 hover:border-primary transition-all"
              >
            ...
            >
              <input {...getInputProps()} />
              <FileUp className="w-12 h-12 min-[2000px]:w-16 min-[2000px]:h-16 mx-auto mb-4 min-[2000px]:mb-6 text-muted-foreground" />
              {file ? (
                <div>
                  <p className="font-medium text-lg min-[2000px]:text-xl">
                    {file.name}
                  </p>
                  <p className="text-sm min-[2000px]:text-base text-muted-foreground">
                ...
                <div>
                  ...
                  <p className="text-sm min-[2000px]:text-base text-muted-foreground mt-1">
            ...
            {progress.status === "validating" && (
              <div className="space-y-2 min-[2000px]:space-y-3">
                ...
                />
                <p className="text-sm min-[2000px]:text-base text-center text-muted-foreground animate-pulse">
                  {progress.message}
                </p>
              </div>
            )}
          ...
          <div className="space-y-4 min-[2000px]:space-y-6">
            <div className="grid grid-cols-3 gap-4 min-[2000px]:gap-6">
              ...
              <Card className="border-red-200 bg-gradient-to-br from-red-50/50 to-transparent">
                <CardContent className="pt-6 min-[2000px]:pt-8">
                  <div className="flex items-center gap-3 min-[2000px]:gap-4">
                    <div className="p-2 min-[2000px]:p-3 bg-red-100 rounded-lg">
              ...
              <Card className="border-yellow-200 bg-gradient-to-br from-yellow-50/50 to-transparent">
                <CardContent className="pt-6 min-[2000px]:pt-8">
                  <div className="flex items-center gap-3 min-[2000px]:gap-4">
                    <div className="p-2 min-[2000px]:p-3 bg-yellow-100 rounded-lg">
            ...
            <Tabs defaultValue="valid" className="w-full">
              <TabsList className="grid w-full grid-cols-3 min-[2000px]:h-12">
                <TabsTrigger
                  value="valid"
                  className="data-[state=active]:bg-green-100 data-[state=active]:text-green-700 min-[2000px]:text-base"
                >
                  Valid ({preview.valid.length})
                </TabsTrigger>
                <TabsTrigger
                  value="invalid"
                  className="data-[state=active]:bg-red-100 data-[state=active]:text-red-700 min-[2000px]:text-base"
                >
                  Invalid ({preview.invalid.length})
                </TabsTrigger>
                <TabsTrigger
                  value="duplicates"
                  className="data-[state=active]:bg-yellow-100 data-[state=active]:text-yellow-700 min-[2000px]:text-base"
                >
              ...
              <TabsContent value="valid">
                <ScrollArea className="h-[300px] min-[2000px]:h-[400px] w-full rounded-lg border">
                  <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                      {preview.valid.map((row, index) => (
                        <TableRow key={index} className="hover:bg-muted/30">
                          ...
                          <TableCell className="min-[2000px]:text-base">
                            {row.phone}
                          </TableCell>
                          <TableCell className="min-[2000px]:text-base">
                            {row.city}
        ...
        {step === "importing" && (
          <div className="space-y-4 min-[2000px]:space-y-6 py-8 min-[2000px]:py-12">
            ...
            <Progress
              ...
              className="h-2 min-[2000px]:h-3"
            />
            <p className="text-sm min-[2000px]:text-base text-center text-muted-foreground">
              {progress.current} of {progress.total} parties imported
            </p>
          </div>
        )}

        <DialogFooter className="min-[2000px]:gap-3">
          {step === "preview" && (
            <>
              <Button
                variant="outline"
                onClick={() => setStep("upload")}
                className="h-10 min-[2000px]:h-12 min-[2000px]:text-base hover:bg-muted"
              >
                Back
              </Button>
              <Button
                ...
                className="h-10 min-[2000px]:h-12 min-[2000px]:text-base bg-gradient-to-r from-primary to-primary/80 hover:shadow-lg transition-all"
              >
          ...
          {step === "upload" && (
            <Button
              variant="outline"
              onClick={onClose}
              className="h-10 min-[2000px]:h-12 min-[2000px]:text-base"
            >
              Cancel
            </Button>
          )}
...
export const Customers = () => {
  ...
  const loadParties = useCallback(async () => {
    ...
    try {
      ...
      const activeParties =
        partiesWithMetrics?.filter((p) => p.status === "ACTIVE").length || 0;
      const consignorParties =
        partiesWithMetrics?.filter(
          (p) => p.party_type === "CONSIGNOR" || p.party_type === "BOTH",
        ).length || 0;
      const consigneeParties =
        partiesWithMetrics?.filter(
          (p) => p.party_type === "CONSIGNEE" || p.party_type === "BOTH",
        ).length || 0;
      const billingParties =
        partiesWithMetrics?.filter((p) => p.is_billing_party === true).length ||
        0;
    ...
    return parties.filter((party) => {
      ...
      const matchesType =
        ...
        (typeFilter === "CONSIGNOR" &&
          (party.party_type === "CONSIGNOR" || party.party_type === "BOTH")) ||
        (typeFilter === "CONSIGNEE" &&
          (party.party_type === "CONSIGNEE" || party.party_type === "BOTH")) ||
  ...
  const handleExport = () => {
    const exportData = filteredParties.map((party) => ({
      ...
      PAN: party.pan_number || "",
      Status: party.status,
      "Billing Party": party.is_billing_party ? "YES" : "NO",
      "Trips This Month": party.trips_this_month || 0,
    }));
    ...
    toast({
      title: "✅ Exported",
      description: `${filteredParties.length} parties exported successfully`,
    });
  };
  ...
  return (
    <div className="space-y-6 min-[2000px]:space-y-8">
      ...
      <div className="flex flex-col lg:flex-row gap-4 min-[2000px]:gap-6 lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-[#E5E7EB] dark:divide-[#35353F]">
            <div className="px-6 min-[2000px]:px-8 py-3 min-[2000px]:py-4 first:pl-0 relative">
              <div className="absolute top-2 min-[2000px]:top-3 right-2 min-[2000px]:right-3 opacity-10">
                <Building2 className="w-8 h-8 min-[2000px]:w-10 min-[2000px]:h-10 text-muted-foreground dark:text-muted-foreground" />
              </div>
            ...
            <div className="px-6 min-[2000px]:px-8 py-3 min-[2000px]:py-4 last:pr-0 relative">
              <div className="absolute top-2 min-[2000px]:top-3 right-2 min-[2000px]:right-3 opacity-10">
                <DollarSign className="w-8 h-8 min-[2000px]:w-10 min-[2000px]:h-10 text-primary" />
              </div>
              <div className="relative z-10">
                <p className="text-xs min-[2000px]:text-sm font-medium text-muted-foreground dark:text-muted-foreground mb-1">
                  Billing
        ...
        <div className="flex flex-col gap-2 min-[2000px]:gap-3 lg:flex-shrink-0 lg:min-w-[200px] min-[2000px]:lg:min-w-[250px]">
          <Button
            size="default"
      ...
      <div className="bg-card border border-border dark:border-border rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="border-b border-border dark:border-border">
          <Tabs value={selectedTab} onValueChange={setSelectedTab}>
            <div className="px-6 min-[2000px]:px-8 overflow-x-auto scrollbar-none">
              <TabsList className="bg-transparent border-0 p-0 h-auto inline-flex min-w-max">
                <TabsTrigger
                  value="all"
                  className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-6 min-[2000px]:px-8 py-3.5 min-[2000px]:py-4 transition-all text-sm min-[2000px]:text-base font-medium text-muted-foreground dark:text-muted-foreground hover:text-foreground dark:hover:text-white"
                >
                ...
                <TabsTrigger
                  ...
                  className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none px-6 min-[2000px]:px-8 py-3.5 min-[2000px]:py-4 transition-all text-sm min-[2000px]:text-base font-medium text-muted-foreground dark:text-muted-foreground hover:text-foreground dark:hover:text-white"
                >
                  <DollarSign className="w-4 h-4 min-[2000px]:w-5 min-[2000px]:h-5 mr-1.5 inline" />
          ...
          <div className="flex flex-col sm:flex-row gap-3 min-[2000px]:gap-4 items-start sm:items-center justify-between">
            ...
            <div className="flex gap-3 min-[2000px]:gap-4 w-full sm:w-auto">
              <Select value={typeFilter} onValueChange={setTypeFilter}>
                <SelectTrigger className="w-full sm:w-[160px] min-[2000px]:sm:w-[200px] h-10 min-[2000px]:h-12 border-border dark:border-border bg-card text-sm min-[2000px]:text-lg">
                  <SelectValue />
                </SelectTrigger>
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent className="bg-card border-border dark:border-border">
                  ...
                  <SelectItem value="ACTIVE" className="min-[2000px]:text-base">
                    Active
                  </SelectItem>
                  <SelectItem
                    value="INACTIVE"
          ...
          <Table className="customers-table">
            <TableHeader>
              <TableRow className="border-b border-border dark:border-border hover:bg-muted dark:hover:bg-[#252530]">
                <TableHead className="w-[30%] font-semibold text-muted-foreground dark:text-muted-foreground pl-6 min-[2000px]:pl-8 min-[2000px]:text-base">
                  <div className="flex items-center gap-2 min-[2000px]:gap-3">
                    <Building2 className="w-4 h-4 min-[2000px]:w-5 min-[2000px]:h-5" />
                    Party Name
                  </div>
                </TableHead>
                <TableHead className="w-[12%] font-semibold text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base">
                  Type
                </TableHead>
                <TableHead className="w-[15%] font-semibold text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base">
                  City / State
                </TableHead>
                <TableHead className="w-[10%] font-semibold text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base">
                  Last Booking
                </TableHead>
                <TableHead className="w-[8%] font-semibold text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base">
                  Trips
                </TableHead>
                <TableHead className="w-[10%] font-semibold text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base">
                  Verified
                ...
                <TableHead className="w-[5%] font-semibold text-muted-foreground dark:text-muted-foreground text-center pr-6 min-[2000px]:pr-8 min-[2000px]:text-base">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {paginatedParties.length === 0 ? (
                  ...
                  <TableCell colSpan={8} className="h-64 text-center">
                    <div className="flex flex-col items-center justify-center gap-3 min-[2000px]:gap-4">
                      <div className="w-16 h-16 min-[2000px]:w-20 min-[2000px]:h-20 rounded-full bg-muted flex items-center justify-center">
                        <Users className="w-8 h-8 min-[2000px]:w-10 min-[2000px]:h-10 text-muted-foreground dark:text-muted-foreground" />
                      </div>
                      <div>
                ...
                paginatedParties.map((party) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell className="pl-6 min-[2000px]:pl-8 font-medium text-foreground dark:text-white min-[2000px]:text-base">
                        <div className="font-semibold">{party.name}</div>
                        {party.contact_person && (
                          <div className="text-xs min-[2000px]:text-sm text-muted-foreground dark:text-muted-foreground mt-0.5">
                            {party.contact_person}
                          </div>
                        )}
                      </TableCell>
                      <TableCell>
                        <Badge
                          className={cn(
                            "gap-1.5 text-xs min-[2000px]:text-sm font-medium",
                            typeConfig.color,
                          )}
                        >
                          <TypeIcon className="w-3 h-3 min-[2000px]:w-4 min-[2000px]:h-4" />
                          {typeConfig.label}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-sm min-[2000px]:text-base text-foreground dark:text-white">
                        {party.city}, {party.state}
                      </TableCell>
                      <TableCell className="text-sm min-[2000px]:text-base text-muted-foreground dark:text-muted-foreground">
                      ...
                      <TableCell>
                        <Badge
                          className={cn(
                            "cursor-pointer transition-all text-xs min-[2000px]:text-sm font-medium",
                            party.status === "ACTIVE"
                        ...
                        >
                          {party.status === "ACTIVE" ? (
                            <CheckCircle className="w-3 h-3 min-[2000px]:w-4 min-[2000px]:h-4 mr-1" />
                          ) : (
                      ...
                      <TableCell className="pr-6 min-[2000px]:pr-8">
                        <DropdownMenu>
                          ...
                          >
                            ...
                            <DropdownMenuItem
                              className="text-[#DC2626] hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer min-[2000px]:text-base min-[2000px]:py-3"
        ...
        {/* Mobile View */}
        <div className="md:hidden p-4 min-[2000px]:p-6 space-y-3 min-[2000px]:space-y-4">
          {paginatedParties.length === 0 ? (
            <div className="text-center py-16 min-[2000px]:py-20">
              <div className="w-16 h-16 min-[2000px]:w-20 min-[2000px]:h-20 rounded-full bg-muted flex items-center justify-center mx-auto mb-4 min-[2000px]:mb-6">
                <Users className="w-8 h-8 min-[2000px]:w-10 min-[2000px]:h-10 text-muted-foreground dark:text-muted-foreground" />
              </div>
              <p className="text-base min-[2000px]:text-lg font-medium text-foreground dark:text-white">
                No parties found
            ...
            paginatedParties.map((party) => {
              ...
              return (
                <div
                  key={party.id}
                  className="bg-card border border-border dark:border-border rounded-lg p-4 min-[2000px]:p-6 space-y-3 min-[2000px]:space-y-4 shadow-sm hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 min-[2000px]:gap-3">
                        <Building className="w-4 h-4 min-[2000px]:w-5 min-[2000px]:h-5 text-muted-foreground dark:text-muted-foreground" />
                        <span className="font-semibold text-sm min-[2000px]:text-base text-foreground dark:text-white">
                          {party.name}
                        </span>
                      </div>
                      {party.contact_person && (
                        <div className="flex items-center gap-1 min-[2000px]:gap-1.5 text-xs min-[2000px]:text-sm text-muted-foreground dark:text-muted-foreground ml-6 min-[2000px]:ml-8 mt-1">
                          <User className="w-3 h-3 min-[2000px]:w-4 min-[2000px]:h-4" />
                          {party.contact_person}
                        </div>
                      )}
                    </div>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-card">
                        <DropdownMenuItem
                          onClick={() => handleEditParty(party.id)}
                          className="min-[2000px]:text-base min-[2000px]:py-3"
                        >
                          <Edit className="mr-2 h-4 w-4 min-[2000px]:h-5 min-[2000px]:w-5" />
                          Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          className="text-[#DC2626] min-[2000px]:text-base min-[2000px]:py-3"
                          onClick={() => setDeletePartyId(party.id)}
                        >
                          <Trash2 className="mr-2 h-4 w-4 min-[2000px]:h-5 min-[2000px]:w-5" />
                          Delete
                  ...
                  <div className="flex items-center gap-2 min-[2000px]:gap-3">
                    <Badge
                      className={cn(
                        "gap-1 min-[2000px]:gap-1.5 text-xs min-[2000px]:text-sm",
                        typeConfig.color,
                      )}
                    >
                      <TypeIcon className="w-3 h-3 min-[2000px]:w-4 min-[2000px]:h-4" />
                  ...
                  <div className="space-y-2 min-[2000px]:space-y-3 pt-2 min-[2000px]:pt-3 border-t border-border dark:border-border">
                    <div className="flex items-center gap-2 min-[2000px]:gap-3 text-sm min-[2000px]:text-base text-muted-foreground dark:text-muted-foreground">
                      <Phone className="w-3.5 h-3.5 min-[2000px]:w-4 min-[2000px]:h-4" />
                      <span>{party.phone}</span>
                    </div>
                    {party.email && (
                      <div className="flex items-center gap-2 min-[2000px]:gap-3 text-sm min-[2000px]:text-base text-muted-foreground dark:text-muted-foreground">
                        <Mail className="w-3.5 h-3.5 min-[2000px]:w-4 min-[2000px]:h-4" />
                        <span className="text-xs min-[2000px]:text-sm truncate">
                          {party.email}
                        </span>
                      </div>
                    )}
                    <div className="flex items-start gap-2 min-[2000px]:gap-3 text-sm min-[2000px]:text-base text-muted-foreground dark:text-muted-foreground">
                      <MapPin className="w-3.5 h-3.5 min-[2000px]:w-4 min-[2000px]:h-4 mt-0.5" />
                      <div className="text-xs min-[2000px]:text-sm">
        ...
        {filteredParties.length > 0 && (
          <div className="px-6 min-[2000px]:px-8 py-4 min-[2000px]:py-6 border-t border-border dark:border-border bg-card">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 min-[2000px]:gap-6">
              ...
              <div className="flex items-center gap-2 min-[2000px]:gap-3">
                {currentPage > 2 && (
                  <Button
                    ...
                    className="h-9 w-9 min-[2000px]:h-11 min-[2000px]:w-11 p-0 border-border dark:border-border bg-card hover:bg-accent dark:hover:bg-secondary text-foreground dark:text-white hover:text-foreground dark:hover:text-white"
                  >
                ...
                <Button
                  ...
                  className="h-9 w-9 min-[2000px]:h-11 min-[2000px]:w-11 p-0 border-border dark:border-border bg-card hover:bg-accent dark:hover:bg-secondary text-foreground dark:text-white hover:text-foreground dark:hover:text-white disabled:opacity-50"
                >
                ...
                <div className="flex items-center gap-1 min-[2000px]:gap-1.5">
                  {getPaginationButtons().map((page, index) =>
                    page === "..." ? (
                      <span
                        key={`ellipsis-${index}`}
                        className="px-2 min-[2000px]:px-3 text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base"
                      >
                        ...
                      </span>
                    ) : (
              ...
              >
                <SelectTrigger className="w-[130px] min-[2000px]:w-[160px] h-9 min-[2000px]:h-11 border-border dark:border-border bg-card text-sm min-[2000px]:text-base text-foreground dark:text-white hover:text-foreground dark:hover:text-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-card border-border dark:border-border">
                  <SelectItem
                    value="10"
                    className="text-foreground dark:text-white hover:text-foreground dark:hover:text-white min-[2000px]:text-base"
                  >
                    10 per page
                  </SelectItem>
                  <SelectItem
        ...
        <AlertDialogContent className="bg-card border-border dark:border-border min-[2000px]:max-w-lg">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted-foreground dark:text-muted-foreground min-[2000px]:text-base">
              ...
              party and remove all associated data from our servers.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="min-[2000px]:gap-3">
            <AlertDialogCancel className="border-border dark:border-border hover:bg-muted dark:hover:bg-secondary h-10 min-[2000px]:h-12 min-[2000px]:text-base">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDeleteParty}
              className="bg-[#DC2626] hover:bg-[#B91C1C] text-white h-10 min-[2000px]:h-12 min-[2000px]:text-base"
            >
              Delete Party
            </AlertDialogAction>
          </AlertDialogFooter>
